...
class SimpleTfKinematics(Node):
    def __init__(self):
        ...
        self.static_transfrom_stamped_ = TransformStamped()
        ...
        self.orientation_increment_ = quaternion_from_euler(0, 0, 0.05) # rad
        
        self.tf_buffer_ = Buffer()
        self.tf_listener_ = TransformListener(self.tf_buffer_, self)
        
        
        self.static_transfrom_stamped_.header.stamp = self.get_clock().now().to_msg()
        self.static_transfrom_stamped_.header.frame_id = "bumperbot_base"
        self.static_transfrom_stamped_.child_frame_id = "bumperbot_top"
        
        self.static_transfrom_stamped_.transform.translation.x = 0.0
        self.static_transfrom_stamped_.transform.translation.y = 0.0
        self.static_transfrom_stamped_.transform.translation.z = 0.3
        self.static_transfrom_stamped_.transform.rotation.x = 0.0
        self.static_transfrom_stamped_.transform.rotation.y = 0.0
        self.static_transfrom_stamped_.transform.rotation.z = 0.0
        self.static_transfrom_stamped_.transform.rotation.w = 1.0
        
        
        self.static_tf_broadcaster_.sendTransform(self.static_transfrom_stamped_)
        
        self.get_logger().info("Publishing static transform between %s and %s" %
                               (self.static_transfrom_stamped_.header.frame_id,
                                self.static_transfrom_stamped_.child_frame_id))
    ...
    def timerCallback(self):
        ...
        if self.rotations_counter_ >= 100: # Change direction after 100 rotations
