...
class SimpleController(Node):    
    ...
    def jointCallback(self, msg):
        ...
        linear = (self.wheel_radius_ * phi_right + self.wheel_radius_ * phi_left) / 2.0
        ...
        prev_theta = self.theta_
        self.theta_ += d_theta
        self.x_ += d_s * math.cos(self.theta_)
        self.y_ += d_s * math.sin(self.theta_)

        # Convert theta_ to degrees
        theta_deg = math.degrees(self.theta_)
        ...
        if (prev_theta < 0 and self.theta_ >= 0) or (prev_theta >= 0 and self.theta_ < 0):
            if theta_deg > 180:
                theta_deg -= 360
            elif theta_deg < -180:
                theta_deg += 360
        ...
        self.theta_ = math.radians(theta_deg)
        ...
        # self.get_logger().info("x: %f, y: %f, theta: %f" % (self.x_, self.y_, theta_deg))
