...
bool SimpleTfKinematics::getTransformCallback(
    const std::shared_ptr<bumperbot_msgs::srv::GetTransform_Request> req,
    const std::shared_ptr<bumperbot_msgs::srv::GetTransform_Response> res)
{
    RCLCPP_INFO_STREAM(get_logger(), "Requested Transfrom between " << req->frame_id << " and " << req->child_frame_id);
    ...
    {
        RCLCPP_ERROR_STREAM(get_logger(), "An error occoured while transforming from : " <<
